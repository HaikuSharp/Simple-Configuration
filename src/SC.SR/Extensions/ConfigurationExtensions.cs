...
{
    public static bool TryGetValueGetter<T>(this IConfiguration configuration, string path, out OptionGetter<T> getter) => (getter = configuration.GetValueGetter<T>(path)) is not null;
    ...
    public static OptionGetter<T> GetValueGetter<T>(this IConfiguration configuration, string path) => configuration.TryGetOption<T>(path, out var option) ? new(option) : null;
